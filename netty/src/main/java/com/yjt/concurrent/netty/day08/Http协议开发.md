...
​    当客户端请求访问服务器时,对访问路径进行检查,检查失败返回403错误,该页面无法访问
​    如果校验通过,以链接的方式打开当前文件的目录,每个目录或文件都是超链接,可以递归访问
​    PS:如果是目录,可以继续递归访问其子目录和文件,如果是文件且可读,则浏览器可以直接打开,或者通过[目标另存为]下载该文件.
​    

   1 服务器添加解码器 

```
HttpRequestDecoder 对请求消息进行解码
HttpResponseEecoder 对响应消息进行编码
HttpObjectAggregator(65536) 将多个消息转换为单一的FullHttpRequest或FullHttpResponse,原因是Http解码器在每个消息中会生成多个消息对象
ChunkedWriteHandler 支持异步消息发送大码流(如大文件)而不占用过多的内存,防止内存溢出
HttpFileHandler 自定义文件处理器
